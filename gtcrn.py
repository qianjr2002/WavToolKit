...
if __name__ == "__main__":
    ...
    x1 = torch.stft(x1, 512, 256, 512, torch.hann_window(
        512).pow(0.5), return_complex=False)
    x2 = torch.stft(x2, 512, 256, 512, torch.hann_window(
        512).pow(0.5), return_complex=False)
    y1 = model(x1)[0]
    y2 = model(x2)[0]
    z1 = torch.complex(y1[..., 0], y1[..., 1])
    z2 = torch.complex(y2[..., 0], y2[..., 1])
    y1 = torch.istft(z1, 512, 256, 512, torch.hann_window(512).pow(0.5))
    y2 = torch.istft(z2, 512, 256, 512, torch.hann_window(512).pow(0.5))
